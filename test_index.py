"""DOM validation test suite for ORGAN-V: Logos landing page."""
...
def test_document_metadata(soup):
    ...
    assert soup.title.string == "ORGAN-V: Logos — Public Discourse & Essays"
    ...
    assert "ORGAN-V: Logos" in meta_desc["content"]
...
def test_navigation_and_organ_links(soup):
    ...
    assert active_links[0]["href"] == "https://organvm-v-logos.github.io/"
...
def test_hub_section_and_system_links(soup):
    ...
    assert h1.text.strip() == "ORGAN-V: Logos — Public Discourse & Essays"
...
def test_css_integrity_and_breakpoints(soup):
    ...
    assert "--primary: #0d47a1" in css_content
